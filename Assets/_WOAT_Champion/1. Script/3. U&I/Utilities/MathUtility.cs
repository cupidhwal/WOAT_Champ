        ...
        public static void ReArrObjects(float r, List<GameObject> objectList)
        {
            ...
            for (int i = 0; i < objectList.Count; i++)
            {
                ...
                // 홀수(count % 2 == 1) 인덱스
                    ...
                    objRect.anchoredPosition = GetCirclePos(center, 2 * r, angle * i + randomAngleOffset);
            }
        }

        // 타원 두 개를 이용하는 랜덤배치
        public static void ReArrObjects(float a, float b, List<GameObject> objectList)
        {
            float angle = (2 * Mathf.PI) / objectList.Count; // 0 ~ 2*PI 사이의 각도 계산
            Vector2 center = new(0, 0);
            ...
            for (int i = 0; i < objectList.Count; i++)
            {
                ...
                // 짝수(count % 2 == 0) 인덱스
                if (i % 2 == 0)
                    objRect.anchoredPosition = GetEllPos(center, a, b, angle * i + randomAngleOffset);

                // 홀수(count % 2 == 1) 인덱스
                else
                    objRect.anchoredPosition = GetEllPos(center, 2 * a, 3 * b, angle * i + randomAngleOffset);
            }
        }
